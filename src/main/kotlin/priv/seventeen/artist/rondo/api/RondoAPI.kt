...
object RondoAPI {
    ...
    fun hasBalance(player: UUID, currencyId: String, amount: BigDecimal): Boolean {
        return CurrencyRegistry.isRegistered(currencyId) &&
    ...
    fun deposit(player: UUID, currencyId: String, amount: BigDecimal, source: String): Boolean {
        ...
        if (!MoneyConstraints.isStorable(newBalance)) return false
        ...
        val committed = AccountManager.deposit(player, currency.id, amt)
        val balanceAfter = committed.getCommittedBalance(player, currency.id)?.balance
        if (committed.success && balanceAfter != null) {
    ...
    fun withdraw(player: UUID, currencyId: String, amount: BigDecimal, source: String): Boolean {
        ...
        val committed = AccountManager.withdraw(player, currency.id, amt)
        val balanceAfter = committed.getCommittedBalance(player, currency.id)?.balance
        if (committed.success && balanceAfter != null) {
            LogManager.submit(TransactionLog(
                playerUuid = player,
    ...
    fun setBalance(player: UUID, currencyId: String, amount: BigDecimal, source: String): Boolean {
        ...
        val committed = AccountManager.setBalance(player, currency.id, amt)
        val balanceAfter = committed.getCommittedBalance(player, currency.id)?.balance
        if (committed.success && balanceAfter != null) {
            LogManager.submit(TransactionLog(
                playerUuid = player,
    ...
    fun transfer(from: UUID, to: UUID, currencyId: String, amount: BigDecimal): TransferResult {
        ...
        if (!currency.negativeAllowed && !hasBalance(from, currency.id, totalCost)) {
            return TransferResult(false, "余额不足", taxAmount = taxAmount)
        }
        val event = EconomyTransferEvent(from, to, currency, amt, taxAmount)
        Bukkit.getPluginManager().callEvent(event)
        if (event.isCancelled) {
            return TransferResult(false, "转账被取消", taxAmount = taxAmount)
        }
        ...
        if (!atomic.success) {
            ...
            return TransferResult(false, message, taxAmount = taxAmount)
        }

        val fromAfter = atomic.getCommittedBalance(from, currency.id)?.balance
        val toAfter = atomic.getCommittedBalance(to, currency.id)?.balance
        if (fromAfter != null) {
            LogManager.submit(TransactionLog(
                playerUuid = from,
                currencyId = currency.id,
